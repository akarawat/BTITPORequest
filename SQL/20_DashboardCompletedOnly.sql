-- ============================================================
-- Script  : 20_DashboardCompletedOnly.sql
-- Purpose : แก้ไข ITPO_sp_GetDashboard
--           - TotalAmount (Result Set 1) → นับเฉพาะ Status = 6 (Completed)
--           - Daily Amount (Result Set 2) → นับเฉพาะ Status = 6 (Completed)
--           - Monthly (Result Set 6) → ไม่เปลี่ยน (กรอง Status=6 อยู่แล้ว)
...
BEGIN
    ...
    -- ── (1) Summary counts ────────────────────────────────
    --   TotalAmount = เฉพาะ Status = 6 (Completed)
    SELECT
        COUNT(*)                                                                    AS TotalPO,
        ISNULL(SUM(CASE WHEN Status = 6 THEN GrandTotal ELSE 0 END), 0)            AS TotalAmount,
    ...
    -- ── (2) Daily amounts แยก IT/OS — เฉพาะ Status = 6 (Completed) ──
    SELECT
...
PRINT 'Updated: ITPO_sp_GetDashboard (TotalAmount + Daily chart = Completed only)'
GO

PRINT '=== 20_DashboardCompletedOnly.sql done ==='
GO
